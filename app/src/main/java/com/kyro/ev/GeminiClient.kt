...
Allowed actions: OPEN_YOUTUBE, SEARCH_YOUTUBE, SEARCH_AND_PLAY_LATEST, PLAY, PAUSE, RESUME, NEXT, BACK, SCROLL_DOWN, NONE.
For SEARCH_YOUTUBE include a string field named query.
For SEARCH_AND_PLAY_LATEST include a string field named query. Use this action when the user asks to search YouTube and then play/open the latest or first result, such as 'open YouTube and search Minecraft and play latest video'.
For all other actions omit query.
        ...
        for (model in models) {
            for (attempt in 0..2) {
                try { return request(model, prompt) }
                    ...
                    if (attempt < 2) Thread.sleep(1500L * (1L shl attempt))
                }
